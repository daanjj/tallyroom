...
- **Open saved results** (lobby) loads a JSON you saved earlier. It opens in review mode: the presenter screen shows that run's questions and distributions, you can page through the questions, and nothing is written to the live poll, so a session in progress is safe. **Publish to the room** replaces the live questions and answers with the imported ones for everyone; **Exit review** puts your screen back as it was. CSV is export only, since it cannot be read back without guessing.
